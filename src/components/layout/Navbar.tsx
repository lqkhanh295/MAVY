  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-200 ${isScrolled
        ? "bg-[#00153d]/85 backdrop-blur-md border-b border-white/[0.08] py-3 shadow-lg"
        : "bg-[#00153d] border-b border-white/[0.05] py-4"
        }`}
    >
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <Link href="/" className="flex items-center gap-3.5 shrink-0 group">
            ...
            <div>
              <span className="text-xl sm:text-2xl font-black tracking-tight text-white block leading-none">
                MAVY <span className="text-coral">SEAFOOD</span>
              </span>
          ...
          <nav className="hidden lg:flex items-center gap-5 xl:gap-7">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-xs xl:text-sm font-semibold text-ink-light/90 hover:text-coral transition-colors whitespace-nowrap"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="hidden sm:flex items-center gap-3 shrink-0">
            <a
              ...
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white/[0.06] border border-white/10 text-xs font-bold text-coral hover:bg-white/10 transition-colors whitespace-nowrap"
            >
              <IoCallOutline className="w-4 h-4 text-coral" />
              <span>Zalo 090 132 5178</span>
            </a>

            <a
              href="#products"
              className="px-4 py-2 rounded-xl bg-coral text-navy-950 text-xs font-bold hover:bg-coral-hover transition-colors whitespace-nowrap shadow"
            >
              Bảng Giá Hải Sản
            </a>
          </div>
